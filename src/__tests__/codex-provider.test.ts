...
describe('CodexProvider', () => {
  ...
  it('maps web_search item to tool_use + tool_result', async () => {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());
    ...
    assert.equal(toolUse.name, 'WebSearch');
    assert.equal(toolUse.input.query, 'latest codex sdk docs');
    assert.equal(toolResult.tool_use_id, 'web-1');
    assert.match(toolResult.content, /Search finished/);
  });

  it('maps todo_list item to task_update status event', async () => {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());
    ...
    (provider as any).handleCompletedItem(mockController, {
      ...
      items: [
        ...
        { text: 'Patch bridge', completed: false },
      ],
    });

    const events = parseSSEChunks(chunks);
    assert.equal(events[0].type, 'task_update');
    const taskUpdate = JSON.parse(events[0].data);
    assert.equal(taskUpdate.session_id, 'codex');
    assert.equal(taskUpdate.todos.length, 2);
  });
  ...
  it('creates a provider-specific Codex client when provider is selected', async () => {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());
  ...
  it('passes sandbox, approval, and search overrides to Codex', async () => {
    ...
    assert.equal(capturedStartOptions?.webSearchMode, 'live');
