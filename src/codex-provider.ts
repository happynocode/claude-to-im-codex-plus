...
export class CodexProvider implements LLMProvider {
  ...
  private codexClients = new Map<string, CodexInstance>();
  ...
  private getProviderClientKey(provider?: BridgeApiProvider): string {
    return provider?.id && provider.id !== 'env' ? provider.id : 'env';
  }

  /**
   * Lazily load the Codex SDK. Throws a clear error if not installed.
   */
  private async ensureSDK(provider?: BridgeApiProvider): Promise<{ sdk: CodexModule; codex: CodexInstance }> {
    const clientKey = this.getProviderClientKey(provider);
    const existing = this.codexClients.get(clientKey) || (clientKey === 'env' ? this.codex : null);
    if (this.sdk && existing) {
      return { sdk: this.sdk, codex: existing };
    }
    ...
    const providerApiKey = typeof provider?.apiKey === 'string' ? provider.apiKey : undefined;
    const providerBaseUrl = typeof provider?.baseUrl === 'string' ? provider.baseUrl : undefined;
    const providerModelProvider = typeof provider?.modelProvider === 'string' ? provider.modelProvider : undefined;

    // Resolve API key: provider override > CTI_CODEX_API_KEY > CODEX_API_KEY > OPENAI_API_KEY > (login auth)
    const apiKey = clientKey === 'env'
      ? (process.env.CTI_CODEX_API_KEY || process.env.CODEX_API_KEY || process.env.OPENAI_API_KEY || undefined)
      : providerApiKey;
    const baseUrl = clientKey === 'env'
      ...
      start(controller) {
        (async () => {
          ...
          try {
            ...
            const approvalPolicy = params.approvalPolicy || toApprovalPolicy(params.permissionMode);
  ...
  private handleCompletedItem(
    ...
    switch (itemType) {
      ...
      case 'web_search': {
        ...
        controller.enqueue(sseEvent('tool_result', {
          tool_use_id: toolId,
          content: query ? `Search finished: ${query}` : 'Search finished',
          is_error: false,
        }));
        break;
      }
