...
} from '../node_modules/claude-to-im/src/lib/bridge/host.js';
import type { ChannelBinding, ChannelType } from '../node_modules/claude-to-im/src/lib/bridge/types.js';
...
interface ParsedCodexProvider extends BridgeApiProvider {
  id: string;
  name?: string;
  baseUrl?: string;
...
function loadCodexProvidersFromConfig(): { defaultProviderId: string | null; providers: Map<string, ParsedCodexProvider> } {
  const providers = new Map<string, ParsedCodexProvider>();
  let defaultProviderId: string | null = null;
  let currentProviderId: string | null = null;

  let content = '';
  try {
    content = fs.readFileSync(getCodexConfigPath(), 'utf8');
  } catch {
  ...
  for (const rawLine of content.split('\n')) {
    ...
    if (sectionMatch) {
      currentProviderId = sectionMatch[1].trim();
      if (!providers.has(currentProviderId)) {
        providers.set(currentProviderId, {
    ...
    if (key === 'name' && typeof value === 'string') {
      provider.name = value;
    } else if (key === 'base_url' && typeof value === 'string') {
...
function extractTomlString(configText: string, key: string): string | undefined {
  const match = configText.match(new RegExp(`^${escapeRegex(key)}\\s*=\\s*"([^"]*)"`, 'm'));
...
function loadCodexProvidersFromCcSwitch(): { defaultProviderId: string | null; providers: Map<string, ParsedCodexProvider> } {
  const providers = new Map<string, ParsedCodexProvider>();
  let defaultProviderId: string | null = null;
  ...
  try {
    output = execFileSync('sqlite3', [
      dbPath,
      "select json_object('id', id, 'name', name, 'is_current', is_current, 'settings_config', settings_config) from providers where app_type='codex' order by id;",
    ], { encoding: 'utf8' });
  } catch {
    return { defaultProviderId, providers };
  }
  ...
  for (const line of output.split('\n')) {
    ...
    let row: { id?: string; name?: string; is_current?: number; settings_config?: string };
    try {
      row = JSON.parse(trimmed) as { id?: string; name?: string; is_current?: number; settings_config?: string };
    } catch {
      continue;
    }
    if (!row.id) continue;

    let settingsConfig: { auth?: Record<string, unknown>; config?: string } | undefined;
    try {
      settingsConfig = row.settings_config ? JSON.parse(row.settings_config) : undefined;
    } catch {
    ...
    const baseUrl = extractProviderBaseUrl(configText, modelProvider);
...
export class JsonFileStore implements BridgeStore {
  ...
  private providers = new Map<string, ParsedCodexProvider>();
  ...
  private loadProviders(): void {
    const ccSwitchState = loadCodexProvidersFromCcSwitch();
    if (ccSwitchState.providers.size > 0) {
      this.providers = ccSwitchState.providers;
      this.defaultProviderId = ccSwitchState.defaultProviderId;
      return;
    }

    const providerState = loadCodexProvidersFromConfig();
    this.providers = providerState.providers;
    this.defaultProviderId = providerState.defaultProviderId;
  }
  ...
  listProviders(): BridgeApiProvider[] {
    return Array.from(this.providers.values()).sort((left, right) => left.id.localeCompare(right.id));
  }
